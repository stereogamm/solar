...
type setSearchParamsFn = {
  ...
  submitHandler: (value: FormData) => void;
};
...
export const UserForm = ({
  ...
  const form = useForm({
    ...
    onValuesChange: (values) => {
      setSearchParamsFn(values);
    },
  });

  const sendData = (value: FormData) => {
    submitHandler(value);
    form.reset();
  };
    ...
    <Paper shadow="md" radius="md">
      <div className={styles.wrapper}>
        <form
          ...
          onSubmit={form.onSubmit((value) => sendData(value))}
        >
          <Text
            variant="gradient"
          ...
          <div className={styles.fields}>
            <SimpleGrid cols={{ base: 1, sm: 2 }}>
              ...
              <TextInput
                ...
                label="Altitude for observer in meter"
                placeholder="Height above sea level (from 0)"
                pointer={true}
                withAsterisk
                key={form.key("elev")}
              />
              <TextInput
                ...
                label="Time Zone for observer"
                placeholder="from -12 to +14"
                pointer={true}
                withAsterisk
                key={form.key("zone")}
              />
